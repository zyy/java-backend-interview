...
### Q1: MySQL 有哪些锁？如何避免死锁？

**答：** MySQL 的锁主要分为以下几类：

#### 1. 按锁的粒度划分
- **表级锁**：锁定整个表，如 `LOCK TABLES`。
- **行级锁**：锁定表中的某一行，InnoDB 支持。
- **页级锁**：锁定一页（16KB），BDB 存储引擎支持。
...
- **间隙锁（Gap Lock）**：锁定索引记录之间的间隙，防止其他事务在此间隙插入。
- **临键锁（Next-Key Lock）**：记录锁 + 间隙锁的组合。
- **意向锁**：表级锁，表示事务打算在表中加行锁。
  - **意向共享锁（IS）**：事务打算在某些行上加共享锁。
  - **意向排他锁（IX）**：事务打算在某些行上加排他锁。

#### 4. 如何避免死锁？
1. **统一加锁顺序**：多个事务按照相同的顺序获取锁，避免循环等待。
2. **减小事务粒度**：尽快提交事务，减少锁持有时间。
3. **使用索引**：确保查询使用索引，减少锁的范围。
4. **设置锁等待超时**：`innodb_lock_wait_timeout` 设置锁等待超时时间。
5. **使用低隔离级别**：在允许的情况下使用 READ COMMITTED 隔离级别，减少间隙锁的使用。
6. **监控与报警**：监控 `SHOW ENGINE INNODB STATUS` 中的死锁信息，及时发现并解决。

#### 5. 死锁检测与处理
- **自动检测**：InnoDB 自动检测死锁，并回滚代价最小的事务。
- **手动分析**：
  ```sql
  SHOW ENGINE INNODB STATUS;  -- 查看最近的死锁信息
  ```
- **预防措施**：合理设计索引，避免全表扫描；批量操作时按固定顺序处理。

**总结**：理解 MySQL 的锁机制是设计高并发应用的基础，合理使用锁和事务隔离级别可以有效避免死锁，保证数据一致性和系统性能。
