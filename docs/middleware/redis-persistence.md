...
### Q1: Redis 的持久化方式有哪些？过期策略是什么？

**答：** Redis 提供了两种主要的持久化方式：RDB 和 AOF，以及多种过期策略。
...
- **缺点**：
  - 可能会丢失最后一次快照之后的数据。
  - 大数据集时，`BGSAVE` 可能会占用较多内存。

#### 2. AOF（Append Only File）
- **原理**：记录每个写操作命令，以日志形式追加到文件末尾。
- **同步策略**：
  - `appendfsync always`：每个写命令都同步到磁盘，最安全但性能最差。
  - `appendfsync everysec`：每秒同步一次，平衡安全性和性能（默认）。
  - `appendfsync no`：由操作系统决定何时同步，性能最好但可能丢失数据。
- **优点**：
  - 数据安全性高，最多丢失一秒的数据（使用 `everysec`）。
  - AOF 文件易于理解和解析。
- **缺点**：
  - 文件体积通常比 RDB 大。
  - 恢复速度比 RDB 慢。
...
#### 3. 混合持久化（Redis 4.0+）
- **原理**：结合 RDB 和 AOF 的优点，先使用 RDB 快照，然后之后的写操作使用 AOF 记录。
- **优势**：既保证了快速恢复，又减少了数据丢失风险。

#### 4. 过期策略
- **定期删除**：Redis 默认每 100ms 随机检查一些设置了过期时间的 key，删除已过期的 key。
- **惰性删除**：当客户端尝试访问一个 key 时，检查是否过期，如果过期则删除。
- **内存淘汰策略**：当内存不足时，Redis 会根据配置的淘汰策略删除 key：
  - `noeviction`：不淘汰，返回错误（默认）。
  ...
  - `volatile-ttl`：淘汰过期时间最短的 key。

#### 5. 如何选择？
- **数据安全性要求高**：使用 AOF（`appendfsync everysec`）或混合持久化。
- **性能要求高，可容忍数据丢失**：使用 RDB。
- **内存敏感**：根据业务特点选择合适的淘汰策略。
- **实际场景**：通常同时开启 RDB 和 AOF，用 RDB 做备份，AOF 保证数据安全。

**总结**：理解 Redis 持久化和过期策略是保证数据安全性和系统性能的关键，需要根据具体业务需求进行合理配置。
