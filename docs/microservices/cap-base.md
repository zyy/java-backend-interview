...
**答：** CAP 理论是分布式系统设计的核心理论，由 Eric Brewer 提出。它指出一个分布式系统最多只能同时满足以下三个特性中的两个：

1. **C (Consistency) - 一致性**：所有节点在同一时间看到的数据完全一致。
2. **A (Availability) - 可用性**：每个请求都能得到响应（不保证是最新数据）。
3. **P (Partition Tolerance) - 分区容错性**：系统在网络分区（节点间无法通信）时仍能继续工作。

**如何取舍？**
- **CA 系统**：放弃 P，要求强一致性和高可用性，适合单机或小型集群（如传统数据库），但无法容忍网络分区。
- **CP 系统**：放弃 A，要求强一致性和分区容错性，在发生网络分区时，系统可能不可用，但保证数据一致（如 ZooKeeper、HBase）。
- **AP 系统**：放弃 C，要求高可用性和分区容错性，在发生网络分区时，系统继续提供服务，但数据可能不一致（如 Cassandra、Eureka）。

**实际应用**：在分布式系统中，P 是必须的（网络分区不可避免），因此实际选择是在 C 和 A 之间权衡。根据业务需求：
- 金融、支付等对一致性要求高的场景 → 偏向 CP
- 社交、电商等对可用性要求高的场景 → 偏向 AP
