...
#### 1. 2PC（两阶段提交）
- **原理**：协调者协调多个参与者，分准备和提交两个阶段。
- **优点**：强一致性。
- **缺点**：同步阻塞、单点故障、数据不一致风险。
- **适用场景**：短事务，对一致性要求极高的场景。

#### 2. TCC（Try-Confirm-Cancel）
- **原理**：业务层面实现 Try（预留）、Confirm（确认）、Cancel（取消）三个阶段。
- **优点**：性能较好，最终一致，不长期锁定资源。
- **缺点**：侵入性强，需要编写三个接口，业务复杂度高。
- **适用场景**：长事务，对性能要求较高的场景。

#### 3. 可靠消息最终一致（消息队列）
- **原理**：通过消息队列保证消息可靠投递，本地事务与消息发送保持一致性。
- **优点**：性能好，系统解耦，最终一致。
- **缺点**：存在延迟，需要处理消息重复消费。
- **适用场景**：异步场景，高并发，允许最终一致的业务。

#### 4. Saga 模式
- **原理**：将长事务拆分为多个本地事务，每个事务有对应的补偿操作。
- **优点**：适合复杂业务流程，避免长事务锁定。
- **缺点**：补偿逻辑复杂，可能产生脏数据。
- **适用场景**：跨多个服务的复杂业务流程。

#### 5. Seata 框架
- **AT 模式**：自动补偿，基于 SQL 逆向操作。
- **TCC 模式**：手动补偿，需要编写三个接口。
- **Saga 模式**：状态机编排长事务。
- **XA 模式**：强一致性，基于 XA 协议。

**选型建议**：根据业务对一致性、性能和复杂度的要求选择合适的方案。
